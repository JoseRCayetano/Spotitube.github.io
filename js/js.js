...
var artistNameSelected;
var player;
var array_favourite = [];

loadLocalStorage("favourite");

//Load user logged
$("#profile strong").text(window.localStorage.getItem("spotyUser"));

searchUserInLocalStorage(window.localStorage.getItem("spotyUser"));
function searchUserInLocalStorage (name){
    var findit = JSON.parse(window.localStorage.getItem("favourite")).map(function (obj){
        return obj.name;
    }).indexOf(name);
    //If not exists create a new object a
    if (findit === -1){
        var userToLocalStorage = function (name) {
            this.name = name,
            this.favouriteTrack = []
        };
        var user = new userToLocalStorage (name);
        array_favourite.push(user);
        saveLocalStorage("favourite");
    }
    /*
    var array_names = .map(function (object){
        return object.name;
    })*/
}
function loadLocalStorage (localName){
   ...
   if (window.localStorage.getItem(localName) == null){
	...
	array_favourite = JSON.parse(window.localStorage.getItem(localName));
    }  
}
function saveLocalStorage (localName) {
    window.localStorage.setItem(localName, JSON.stringify(array_favourite)); 
}
function saveFavourite (localName,userName, idTrack){
    //Si no tiene elementos, crea objeto nuevo con el nombre y la lista de favoritos
    if (array_favourite.length === 0){
        var userToLocalStorage = function (userName) {
            this.name = userName,
            this.favouriteTrack = []
        };
        var user = new userToLocalStorage (userName);
        user.favouriteTrack.push(idTrack);
        array_favourite.push(user);
        //saveLocalStorage("favourite",array_favourite);
        
    }else{
        //Busco si esta el usuario 
        var index = array_favourite.map(function(el) {
            return el.name;
        }).indexOf(userName);
      //Lo ha encontrado
      if (index !== -1){
         array_favourite[index].favouriteTrack.push(idTrack); 
      }else{ //No lo encuentra
          var userToLocalStorage = function (userName) {
            this.name = userName,
            this.favouriteTrack= []
        };
        userToLocalStorage.favouriteTrack.push(idTrack);
        array_favourite.push(userToLocalStorage);
        
      }
    }
    saveLocalStorage(localName,array_favourite);
}
...
function request (url,action,id_carousel) {
  $.ajax({
    url: url
      ...
      }else if (action === "load_favourites") {
          formatFavourites(response);
      }
...
function load_carousel_artist (response,id_carousel){
    ...
    $(id_carousel+" ul li").unbind('click').click(function (e){//Function para cada foto
        ...
        artistNameSelected = $(this).find("h4").text();
...
function init (){
    gapi.client.setApiKey("AIzaSyCNeDFnzL9skMjEGvLCZIr4Tlqkf0BTo_8");
...
function loadVideo(id){
    $("#modal modal-body iframe#player").remove();
    $("#modal modal-body").append('<div id="player"></div>');
